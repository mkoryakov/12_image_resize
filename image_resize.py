...
def get_new_filepath(filepath, width, height):
    root, extension = os.path.splitext(filepath)
    root += '__%dx%d' % (width, height)
    return root + extension


def resize_image(path_to_original, path_to_result,
                 new_scale, new_width, new_height):
    if new_scale and (new_width or new_height):
    ...
    if new_scale:
        ...
        if new_width / new_height != old_width / old_height:
            warning = '''Введённые пропорции width/height не совпадают
    ...
    if not path_to_result:
        path_to_result = get_new_filepath(path_to_original,
...
if __name__ == '__main__':
    ...
    path_to_original = arguments.image
    new_scale = arguments.scale
